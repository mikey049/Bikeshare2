...
qplot(x = Test.rows$registered, y= cubistTune2.predict)
rmse(cubistTune2.predict, Test.rows$registered )
RMSLESummary(data.frame(obs= Test.rows$registered, pred = cubistTune2.predict))

cubist.final <-data.frame(obs= Test.rows, pred = cubistTune2.predict)
#------------------------------------------------------------------------------------------------


gbmTune3 <- train(registered ~ holiday + temp + humidity + season + windspeed + dayofweek + year + peake + peakm + hour + month + (shour.nonwork + workingday + I(weather <3) + shour +smonth)^2 , 
                  data = Training.rows,
                  method = "gbm",
                  metric = "RMSLE",
                  maximize = FALSE,
...
cubistTune3 <- train(casual ~ holiday + workingday + temp + season + windspeed  + year + hour + month  + I(weather <3) + sin((hour-10)*pi/9) + sin((month-8)*pi/3), 
                    data = Training.rows,
                    method = "cubist",
                    metric = "RMSLE",
                    maximize = FALSE,
...
cubistTune4 <- train(casual ~ holiday + workingday + temp + season + windspeed  + year + hour + month  + I(weather <3) + sin((hour-10)*pi/9) + sin((month-8)*pi/3), 
                     data = Training.rows,
                     method = "cubist",
                     tuneGrid = cbGrid,
                     trControl = ctrl5)
...
cubistTune4.predict <-predict(cubistTune4,Test.rows)
rmse(cubistTune4.predict, Test.rows$casual )
RMSLESummary(data.frame(obs= Test.rows$casual, pred = abs(cubistTune4.predict)))

gbmTune4 <- train(casual ~ holiday + workingday + temp + season + windspeed  + year + hour + month  + I(weather <3) + sin((hour-10)*pi/9) + sin((month-8)*pi/3), 
                  data = Training.rows,
                  method = "gbm",
                  metric = "RMSLE",
                  maximize = FALSE,
